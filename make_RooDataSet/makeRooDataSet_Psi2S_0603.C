...
void makeRooDataSet_Psi2S_0603(bool isMC = true, bool fAccW = false, bool fEffW = false, int state=2) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
  //Basic Setting
  gStyle->SetOptStat(0);
  setTDRStyle();
...
//  TFile *fAcc = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Acceptance/acceptance_wgt_%dS_pt0_50_20190813_dNdptWeighted.root",state),"read");
 ...
 // cout << "Average Eff : " << (avgEff_4+avgEff_5+avgEff_6)/3 << endl;
  ...
  else if (!isMC) {TFile *wf = new TFile(Form("./skimmedFiles/OniaRooDataSet_isMC%d_JPsi_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210427.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
 dataSet->Write();
}
