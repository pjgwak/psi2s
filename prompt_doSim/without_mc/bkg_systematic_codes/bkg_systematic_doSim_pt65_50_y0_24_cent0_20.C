...
Double_t TotalYield(Double_t* x, Double_t* par)
{
    ...
    Double_t BkgM = Nbkg*(1+cheb0*shx + cheb1*(2*shx*shx-1));
...
void bkg_systematic_doSim_pt65_50_y0_24_cent0_20(int cLow = 0, int cHigh = 20,
		...
		int ibkg_vn_sel = fpol1, bool fixSigPar=true)
{
	...
	TFile *wf = new TFile(Form("roots/bkg_systematic_SimFitResult_Prompt_%s.root", kineLabel.Data()),"recreate");
	...
	TFile* f_mass = new TFile(Form("../prompt_2D_fit/roots/mass/bkg_systematic_MassFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1.root",kineLabel_.Data()),"read");
	...
	Double_t cheb0_ = 2.0521;
	Double_t cheb1_ = 2.0535;
	Double_t cheb2_ = 4.0546;
	Double_t c_  = 0.161;
	Double_t c1_ = 0.2464;
	Double_t c2_ = 0.0326;
	...
	Double_t parLimitLow[nParmV]  = {    0,       0, mean_ -0.02,     0.01,   1.3,   1.4,    0,     0,  -30, -10, -30,  0, -30, -30, -30,-30};
	Double_t parLimitHigh[nParmV] = {N1_*3, Nbkg_*4, mean_ +0.02,      0.2,    5.1,   4.4,   1  ,   1,   30,  30,  30, 0.5,  30,  30,  30, 30};
	...
	pad2->Draw();
	c_mass_v2->Update();
	c_mass_v2->SaveAs(Form("figs/bkg_systematic_v2Mass_Prompt_%s.pdf", kineLabel.Data()));
	wf->cd();
