        ...
        int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
    ...
    TFile *rf;
    if(isMC){
        //    if(state==1) rf = new TFile("../primary_inputs/OniaFlowSkim_JpsiTrig_isMC1_Prompt_HFNom_200407.root","read");
        // if(state==1) rf = new TFile("../primary_inputs/OniatreeMC_Psi2S_pThat-2_TuneCP5_HydjetDrumMB_5p02TeV_pythia8.root","read");
        if(state==1) rf = new TFile("../../primary_inputs/OniaFlowSkim_Psi2S_JpsiTrig_Prompt_isMC1_HFNom_210603.root","read");
        else if(state==2) rf = new TFile("../../primary_inputs/OniaFlowSkim_JpsiTrig_isMC1_NonPrompt_HFNom_200407.root","read");
    }
    //else if(!isMC) rf = new TFile("./skimmedFiles/OniaFlowSkim_JpsiTrig_AllPD_isMC0_HFNom_200407.root","read");
    else if(!isMC) rf = new TFile("../../primary_inputs/OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_201127.root","read");
    ...
    hEffPt1[4] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy0_2p4",isTnP,isPtW));
    ...
    hEffPt2[4] = (TH1D*) fEff2 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_20_to_120_absy0_2p4",isTnP,isPtW));

    hEffPt2[4]->Draw();
    // TFile *fEff = new TFile(Form("../primary_inputs/mc_eff_vs_pt_cent_20_to_120_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP),"read");
    // cout << "File Open : " << Form("../primary_inputs/mc_eff_vs_pt_cent_20_to_120_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP) << endl;
    // TH1D* hEffPt[15];
    ...
    //TFile *fAcc = new TFile("../../acceptance_Prompt_GenOnly_wgt1_2021Psi2Sv2_20210603.root","read");
    TFile *fAcc = new TFile("../../primary_inputs/acceptance_Prompt_GenOnly_wgt1_2021Psi2Sv2_20210913.root","read");
    ...
    //tree -> SetBranchAddress("TnPweight", &TnPweight, &b_TnPweight);
    ...
    for(int i=0; i<nEvt; i++){
        ...
        if(fabs(vz)<15) {
            ...
            for(int j=0; j<nDimu; j++){
                ...
                if( ((double)pt[j]<50 && abs((double)y[j])<2.4 && IsAcceptanceQQ(pt1[j],eta1[j])&&IsAcceptanceQQ(pt2[j],eta2[j])) ) {
                    ...
                    if(fEffW){
                        if(cBin >= 0 && cBin < 20) {
                            if ( abs((double)y[j])<2.4) { weight_eff = getEffWeight(hEffPt1[4], pt[j]); }
                            ...
                            if ( abs((double)y[j])<2.4) { weight_eff = getEffWeight(hEffPt2[4], pt[j]); }
                    ...
                    double weight_ = weight[j] * weight_eff * weight_acc;
        ...
        if (isMC && state==1) {TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_PR_Psi_2S_%s.root",isMC,DATE.Data()),"recreate");  wf->cd();}
        else if (isMC && state==2) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_BtoPsi_2S_%s.root",isMC,DATE.Data()),"recreate");  wf->cd();}
        else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_Psi_2S_%sw_Effw%d_Accw%d_PtW%d_TnP%d_%s.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP,DATE.Data()),"recreate");  wf->cd();}
        dataSet->Write();
    }
