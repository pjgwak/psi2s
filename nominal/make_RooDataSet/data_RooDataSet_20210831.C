...
void data_RooDataSet_20210831(
		...
		float ctauCut=0.1,
    bool isPtW = true, bool isPtWAcc = true, bool isTnP = true,
		bool isMC = false, 
		...
		int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
  ...
  TFile *rf;
  if(isMC){
    if(state==1) rf = new TFile("../../primary_inputs/OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_201127.root","READ");
...
//  TFile *fEff = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Efficiency/mc_eff_vs_pt_TnP%d_PtW1_OfficialMC_Y%dS_muPtCut3.5.root",isTnP,state),"read");
  ...
  hEffPt1[4] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy0_0p6",isTnP,isPtW));
  ...
  hEffPt2[4] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy0_0p6",isTnP,isPtW));
  ...
  TFile *fAcc = new TFile(Form("../../primary_inputs/acceptance_Prompt_GenOnly_wgt%d_2021Psi2Sv2_20210601.root",isPtWAcc),"read");
  ...
  for(int i=0; i<nEvt; i++){
	  ...
	  if(fabs(vz)<15) {
		  ...
		  for(int j=0; j<nDimu; j++){
			  ...
			  //if(! ((double)pt[j]<50 && abs((double)y[j])<2.4 && (double)pt1[j]>SiMuPtCut&&(double)pt2[j]>SiMuPtCut&&abs((double)eta1[j])<2.4&&abs((double)eta2[j])<2.4)) continue;
			  if( ((double)pt[j]<50 && abs((double)y[j])<2.4 && recoQQsign[j]==0 && mass[j]>massLow&&mass[j]<massHigh&& IsAcceptanceQQ(pt1[j],eta1[j])&&IsAcceptanceQQ(pt2[j],eta2[j])) ) { 
			  //if( ((double)pt[j]<50 && abs((double)y[j])<2.4) ) { 
				  ...
				  if(fEffW){
					  if(cBin >= 0 && cBin < 20) {
						  ...
						  if ( abs((double)y[j])<1.2) { 
                if(isPtW == false && isTnP == true) weight_eff = getEffWeight(hEffPt2[4], pt[j]); 
                else weight_eff = getEffWeight(hEffPt2[0], pt[j]);
              }
 ...
 // cout << "Average Eff : " << (avgEff_4+avgEff_5+avgEff_6)/3 << endl;
  ...
  if (isMC && state==1) {TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_pt_%0.1f_%0.1f_y_%0.1f_%0.1f_Cent_%d_%d_CtauCtu_%0.4f_PR_Psi_2S_%s.root",isMC,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,ctauCut,DATE.Data()),"recreate");  wf->cd();}
  else if (isMC && state==2) {TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_BtoPsi_2S_%s.root",isMC,DATE.Data() ),"recreate");  wf->cd();}
  //else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_Psi_2S_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210604.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
	  ...
	  TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_pt_%0.1f_%0.1f_y_%0.1f_%0.1f_Cent_%d_%d_CtauCtu_%0.4f_wPt%d_wAccPt%d_wTnPPR%d_Psi_2S_%s.root",isMC,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,ctauCut,isPtW,isPtWAcc,isTnP,DATE.Data()),"recreate");
	  wf->cd();
  }
  dataSet->Write();
}
