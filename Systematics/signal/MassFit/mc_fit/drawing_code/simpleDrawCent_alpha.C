void simpleDrawCent_alpha(){
    gROOT->Macro("./rootlogon.C");

	double xbin[] = {5, 35};
    double xerr[] = {5, 25};
	double ybin1[] = {1.8222, 1.9435};
	double yerr1[] = {0.0002, 0.0007};
    ...
    TGraphErrors *v2cent2 = new TGraphErrors(1, xbin2, ybin2, xerr2, yerr2);
	TH1F *hPad = new TH1F("hPad",";Centrality (%);#alpha", 10, 0.0, 100.0);
    ...
    lt1->DrawLatex(0.2,0.76, Form("avg. %.4f", average));
    lt1->DrawLatex(0.2,0.70, Form("int. %.4f", ybin2[0]));
	...
	c1->SaveAs("plot_jpsi_mc_cent_alpha.pdf");
}
