...
void makeRooDataSet_Psi_2S_PR_20210901(
		...
		int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
  ...
  TFile *rf;
  if(isMC){
    if(state==1) rf = new TFile("../../OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_201127.root","READ");
...
//  TFile *fEff = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Efficiency/mc_eff_vs_pt_TnP%d_PtW1_OfficialMC_Y%dS_muPtCut3.5.root",isTnP,state),"read");
  TFile *fEff1 = new TFile(Form("../SysAccEff_files/mc_eff_vs_pt_cent_0_to_20_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP),"read");
  TFile *fEff2 = new TFile(Form("../SysAccEff_files/mc_eff_vs_pt_cent_20_to_120_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP),"read");
  TH1D* hEffPt1[15];
  TH1D* hEffPt2[15];
  ...
  TFile *fAcc = new TFile(Form("../SysAccEff_files/acceptance_Prompt_GenOnly_wgt%d_2021Psi2Sv2_20210601.root",isPtWAcc),"read");
 ...
 // cout << "Average Eff : " << (avgEff_4+avgEff_5+avgEff_6)/3 << endl;
  ...
  //else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_Psi_2S_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210604.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
	  ...
	  TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_pt_%0.1f_%0.1f_y_%0.1f_%0.1f_Cent_%d_%d_CtauCtu_%0.4f_wPt%d_wAccPt%d_wTnPPR%d_Psi_2S_20210831.root",isMC,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,ctauCut,isPtW,isPtWAcc,isTnP),"recreate");  
	  wf->cd();
  }
  dataSet->Write();
}
