...
*/
  fData = new TFile("../../OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_201127.root");
  fPRMC = new TFile("../../ctau_eff/OniaFlowSkim_Psi2S_JpsiTrig_Prompt_isMC1_HFNom_210603.root");
  fNPMC = new TFile("../../ctau_eff/OniaFlowSkim_Psi2S_JpsiTrig_NonPrompt_isMC1_HFNom_210603.root");
...
//  mCut = ("mass>=3.4&&mass<=4.0");
  ...
  for(int bins=0; bins<h_decayPRMC->GetNbinsX(); bins++){
    if(PRMC==0){
      ...
      if(h_decayPRMC->Integral(0,bins)<(0.899*totalPRMC*loose)||h_decayPRMC->Integral(0,bins)>(totalPRMC*0.9099*loose)) continue;
      ...
      cout<<bins<<"th bin"<<", ljpsi: "<<h_decayNPMC->GetBinCenter(bins)<<", NPR eff: "<<h_decayNPMC->Integral(0,bins)/h_decayNPMC->GetEntries()*100<<"%"<<endl;
