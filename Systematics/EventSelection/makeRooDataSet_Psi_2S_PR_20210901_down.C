...
void makeRooDataSet_Psi_2S_PR_20210901_down(
		...
		int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
  ...
  TFile *rf;
  if(isMC){
    if(state==1) rf = new TFile("../../OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFDown_210729.root","READ");
...
//  TFile *fEff = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Efficiency/mc_eff_vs_pt_TnP%d_PtW1_OfficialMC_Y%dS_muPtCut3.5.root",isTnP,state),"read");
 ...
 // cout << "Average Eff : " << (avgEff_4+avgEff_5+avgEff_6)/3 << endl;
  ...
  //else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_Psi_2S_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210604.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
	  ...
	  TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_pt_%0.1f_%0.1f_y_%0.1f_%0.1f_Cent_%d_%d_CtauCtu_%0.4f_wPt%d_wAccPt%d_wTnPPR%d_Psi_2S_Down_20210901.root",isMC,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,ctauCut,isPtW,isPtWAcc,isTnP),"recreate");  
	  //TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_pt_%0.1f_%0.1f_y_%0.1f_%0.1f_Cent_%d_%d_CtauCtu_%0.4f_wPt%d_wAccPt%d_wTnPPR%d_Psi_2S_Up_20210901.root",isMC,ptLow,ptHigh,yLow,yHigh,cLow,cHigh,ctauCut,isPtW,isPtWAcc,isTnP),"recreate");  
	  wf->cd();
  }
  dataSet->Write();
}
