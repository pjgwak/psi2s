...
bool mc_MassFit_weight_pt_4p0_6p5_y_1p6_2p4_Cent_20_120(TFile* f1);

void test()
{
    TFile* f1 = new TFile("../make_RooDataSet/roots/OniaRooDataSet_isMC1_PR_Psi_2S_20210727.root", "read");
    
    bool result = false;
    int count = 0;
    do {
        result = mc_MassFit_weight_pt_4p0_6p5_y_1p6_2p4_Cent_20_120(f1);
        // True: get good result after fitting
        count++;
...
bool mc_MassFit_weight_pt_4p0_6p5_y_1p6_2p4_Cent_20_120(TFile* f1)
{
    ...
    double sl1_mean = .0558; double sl2_mean = 0.4558; double sl3_mean = -0.0658;
    double N_Jpsi_high = 1300000; double N_Bkg_high = 10000;
    ...
    double paramsupper[8] = {0.5,    5,    5., 1.6, 1,     25.0};
    double paramslower[8] = {0.0001,   0.0001,     0.0001, 0.0001, 0.1,      0.0};
    
	//SIGNAL: initial params
    double weight1 = (rand() % 1)/2 + (rand() % 5) * 0.1;
    double weight2 = (rand() % 5) + (rand() % 5) * 0.1;
    double weight3 = (rand() % 5) + (rand() % 5) * 0.1;
    double weight4 = (rand() % 1)*1.8 + (rand() % 5) * 0.01;
    double weight5 = (rand() % 2)/2 + (rand() % 5) * 0.01;

    /*
    double sigma_1_init = 4.14;
    double x_init = 7.5;
    double alpha_1_init = 8;
    double n_1_init = 7.7;
    double f_init = 0.639;
    */
     
    double sigma_1_init = weight1;
    double x_init = weight2;
    double alpha_1_init = weight3;
    double n_1_init = weight4;
    double f_init = weight5;
	...
	//SIGNAL
	RooRealVar    mean("m_{J/#Psi}","mean of the signal gaussian mass PDF",psi_2S_mass, psi_2S_mass - 0.05, psi_2S_mass + 0.05) ;
	...
	//Build the model
	RooRealVar *N_Jpsi= new RooRealVar("N_Jpsi","inclusive Jpsi signals",1100000, 0, N_Jpsi_high);
    ...
    */
    ...
    if (logY_flag == true) {
        ...
        myPlot2_A->GetYaxis()->SetRangeUser(0,180000);
    }
	...
	TFile* outFile;
	//if (PR==2) {
		outFile = new TFile(Form("roots/mass/%s/mc_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP),"recreate");
		c_A->SaveAs(Form("figs/mass/%s/mc_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
	//}
    ...
    double vN_Jpsi = fitN_Jpsi.getVal();
    double eN_Jpsi = fitN_Jpsi.getError();
    
    double vfitAlpha = fitAlpha.getVal();
    double efitAlpha = fitAlpha.getError();
    
	double vfitFraction = fitFraction.getVal();
    double efitFraction = fitFraction.getError();
    
	double vfitN_1 = fitN_1.getVal();
    double efitN_1 = fitN_1.getError();
    
	double vfitSigma_1 = fitSigma_1.getVal();
    double efitSigma_1 = fitSigma_1.getError();
    
	double vfitX = fitX.getVal();
    double efitX = fitX.getError();
 ...
 /*
    ...
    delete wsmass;
*/
    // (fabs(eN_Jpsi) <= 0.2 && fabs(efitAlpha) <= 0.2 && fabs(efitFraction) <= 0.2 && fabs(efitN_1) <= 0.2 && fabs(efitSigma_1) <= 0.2 && fabs(efitX) <= 0.2)
    

    if (paramsupper[3] != vfitN_1 && fabs(eN_Jpsi/vN_Jpsi) <= 0.3 && fabs(efitAlpha/vfitAlpha) <= 0.3 && fabs(efitFraction/vfitFraction) <= 0.3 && fabs(efitN_1/vfitN_1) <= 0.3 && fabs(efitSigma_1/vfitSigma_1) <= 0.3 && fabs(efitX/vfitX) <= 0.3) {
        cout << "sigma_1_init: " << sigma_1_init << endl;
        cout << "x_init: " << x_init << endl;
        cout << "alpha_1_init: " << alpha_1_init << endl;
        cout << "n_1_init: " << n_1_init << endl;
