...
void mc_MassFit_weight_pt_6p5_50p0_y_0p0_2p4_Cent_0_20(
		float ptLow=6.5, float ptHigh=50.0,
		float yLow=0.0, float yHigh=2.4,
		int cLow=0, int cHigh=20,
        double sl1_mean = 0.08, double sl2_mean = 0.024, double sl3_mean = 0.02,
		double N_Jpsi_high = 1800000, double N_Bkg_high = 90000,
		...
		)
{
    ...
    //Cent.10-20
    double paramsupper[8] = {0.7,    2.5,     2.5,   2.5,   1.0,     25.0};
    ...
    //Build the model
    RooRealVar *N_Jpsi= new RooRealVar("N_Jpsi","inclusive Jpsi signals",1700000, 0, N_Jpsi_high);
    ...
    RooFitResult* fitMass = ws->pdf("pdfMASS_Tot")->fitTo(*dsAB,Save(), Hesse(kTRUE), Timer(kTRUE), Extended(kTRUE), SumW2Error(isWeighted), Range(limit_lower, fit_limit), NumCPU(nCPU));
    ...
    */
    ...
    ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,Name("pdfMASS_tot"), LineColor(kBlack), Range(limit_lower, fit_limit));
    ...
    ws->pdf("cball_1_A")->plotOn(myPlot2_A,Name("cball_1_A"), LineColor(kBlue+2), Normalization(fitFraction.getVal()), Range(limit_lower, fit_limit));
    ws->pdf("cball_2_A")->plotOn(myPlot2_A,Name("cball_2_A"), LineColor(kGreen+2), Normalization(1-fitFraction.getVal()), Range(limit_lower, fit_limit));
    ...
    if (logY_flag == true) {
        ...
        myPlot2_A->GetYaxis()->SetRangeUser(0,330000);
    }
