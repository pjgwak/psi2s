...
void bkg_systematic_MassFit_weight_pt_4p0_6p5_y_1p6_2p4_Cent_20_120_210906(
		float ptLow=4.0, float ptHigh=6.5,
		float yLow=1.6, float yHigh=2.4,
		int cLow=20, int cHigh=120,
		double sl1_mean = 0.004206, double sl2_mean = 0.6032202, double sl3_mean = 0.020582,
		double N_Jpsi_high =18000, double N_Bkg_high = 1410000,
		int PR=0, //0=PR, 1=NP, 2=Inc.
		int PRw=1, bool fEffW = true, bool fAccW = true, bool isPtW = true, bool isTnP = true
		)
{
    TString DATE = "210901_bkg";
    ...
    //Cent.10-20
    double paramsupper[8] = {0.07,    5.,     5, 5, 1.,     25.0};
    double paramslower[8] = {0.01,   0,     0, 0, 0,      -5.};
    
    //SIGNAL: initial params
    double sigma_1_init = 0.0351;
    ...
    //SIGNAL
    RooRealVar    mean("m_{J/#Psi}","mean of the signal gaussian mass PDF",psi_2S_mass, psi_2S_mass -0.02, psi_2S_mass + 0.02 ) ;
    ...
    //Build the model
    RooRealVar *N_Jpsi= new RooRealVar("N_Jpsi","inclusive Jpsi signals",N_Jpsi_high,N_Jpsi_high*.8, N_Jpsi_high*1.2);
    ...
    auto & fitX = (RooRealVar &) fitParams[9];
    ...
    */
    ...
    drawText(Form("sl1 = %.4f #pm %.4f \t sl2 = %.4f #pm %.4f"
                  ,fitSl1.getVal(),fitSl1.getError(),fitSl2.getVal(),fitSl2.getError())
    ...
    TFile* outFile;
    //if (PR==2) {
        outFile = new TFile(Form("roots/mass/%s/bkg_systematic_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP),"recreate");
        c_A->SaveAs(Form("figs/mass/%s/bkg_systematic_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
    //}
