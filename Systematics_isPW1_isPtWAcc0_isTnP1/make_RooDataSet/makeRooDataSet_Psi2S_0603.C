...
void makeRooDataSet_Psi2S_0603(bool isMC = false, bool fAccW = true, bool fEffW = true, int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
  //Basic Setting
  gStyle->SetOptStat(0);
  setTDRStyle();
  ...
  TFile *rf;
  if(isMC){
    if(state==1) rf = new TFile("OniaFlowSkim_Psi2S_JpsiTrig_Prompt_isMC1_HFNom_210603.root","read");
    else if(state==2) rf = new TFile("./OniaFlowSkim_Psi2S_JpsiTrig_NonPrompt_isMC1_HFNom_210603.root","read");
  }
  //else if(!isMC) rf = new TFile("./skimmedFiles/OniaFlowSkim_JpsiTrig_AllPD_isMC0_HFNom_200407.root","read");
  else if(!isMC) rf = new TFile("../../OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_201127.root","read");
//  else if(!isMC) rf = new TFile("OniaFlowSkim_JpsiTrig_JPsi_isMC0_HFNom_200619_RooDataSet_test100k.root","read");
  ...
  //bool isTnP = true; bool isPtW = true; bool isPtWAcc = true; // default
  bool isTnP = true; bool isPtW = false; bool isPtWAcc = true; // systematics
  //bool isTnP = false; bool isPtW = true; // systematics
//  TFile *fEff = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Efficiency/mc_eff_vs_pt_TnP%d_PtW1_OfficialMC_Y%dS_muPtCut3.5.root",isTnP,state),"read");
  TFile *fEff1 = new TFile(Form("mc_eff_vs_pt_cent_0_to_20_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP),"read");
  TFile *fEff2 = new TFile(Form("mc_eff_vs_pt_cent_20_to_120_rap_%s_pbpb_psi2S_PtW%d_tnp%d.root",bCont.Data(),isPtW,isTnP),"read");
  //cout << "File Open : " << Form("./Eff_Acc/mc_eff_vs_pt_cent_20_to_120_rap_%s_pbpb_Jpsi_PtW%d_tnp%d_drawsame2.root",bCont.Data(),isPtW,isTnP) << endl;
  TH1D* hEffPt[15];
  hEffPt[0] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy0_1p2",isTnP,isPtW));
  hEffPt[1] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy1p2_1p6",isTnP,isPtW));
  hEffPt[2] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy1p6_1p8",isTnP,isPtW));
  hEffPt[3] = (TH1D*) fEff1 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_20_absy1p8_2p4",isTnP,isPtW));
  hEffPt[4] = (TH1D*) fEff2 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_20_to_120_absy0_1p2",isTnP,isPtW));
  hEffPt[5] = (TH1D*) fEff2 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_20_to_120_absy1p2_1p6",isTnP,isPtW));
  hEffPt[6] = (TH1D*) fEff2 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_20_to_120_absy1p6_1p8",isTnP,isPtW));
  hEffPt[7] = (TH1D*) fEff2 -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_20_to_120_absy1p8_2p4",isTnP,isPtW));
//  TFile *fAcc = new TFile(Form("/home/deathold/work/CMS/analysis/Upsilon_v2/UpsilonPbPb2018_v2/Acceptance/acceptance_wgt_%dS_pt0_50_20190813_dNdptWeighted.root",state),"read");
  //TFile *fAcc = new TFile("Acc_WeightVariation_20210426.root","read");
  TFile *fAcc = new TFile(Form("acceptance_Prompt_GenOnly_wgt%d_2021Psi2Sv2_20210601.root",isPtWAcc),"read");
  //TFile *fAcc = new TFile(Form("acceptance_Prompt_GenOnly_wgt%d_2021Psi2Sv2_20210601.root",isPtW),"read");
  TH1D* hAccPt[6];
	  ...
	  if(fabs(vz)<15) {
		  ...
		  for(int j=0; j<nDimu; j++){
			  ...
			  if( ((double)pt[j]<50 && abs((double)y[j])<2.4 && IsAcceptanceQQ(pt1[j],eta1[j])&&IsAcceptanceQQ(pt2[j],eta2[j])) ) { 
				  ...
				  if(fEffW && cBin >= 20 && cBin < 120){
					  if ( abs((double)y[j])<1.2) { weight_eff = getEffWeight(hEffPt[4], pt[j]); }
					  else if ( abs((double)y[j])>=1.2 && abs((double)y[j])<1.6 ) { weight_eff = getEffWeight(hEffPt[5], pt[j]); }
					  else if ( abs((double)y[j])>=1.6 && abs((double)y[j])<1.8 ) { weight_eff = getEffWeight(hEffPt[6], pt[j]); }
					  else if ( abs((double)y[j])>=1.8 && abs((double)y[j])<2.4 ) { weight_eff = getEffWeight(hEffPt[7], pt[j]); }
				  }
				  //SumEff=+1./weight_eff;
 ...
 // cout << "Average Eff : " << (avgEff_4+avgEff_5+avgEff_6)/3 << endl;
  ...
  if (isMC && state==1) {TFile *wf = new TFile(Form("skimmedFiles/OniaRooDataSet_isMC%d_Psi2S_20210603.root",isMC),"recreate");  wf->cd();}
  else if (isMC && state==2) {TFile *wf = new TFile(Form("skimmedFiles/OniaRooDataSet_isMC%d_BtoPsi2S_20210603.root",isMC),"recreate");  wf->cd();}
  else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_JPsi_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210831.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
 dataSet->Write();
}
