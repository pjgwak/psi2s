...
void makeRooDataSet_Psi_2S_20210727(
        bool isMC = true,
        ...
        int state=1) //state 0: inclusive, state 1: Prompt, state 2: NonPrompt
{
    ...
    if(isMC){
        ...
        // if(state==1) rf = new TFile("../primary_input/OniatreeMC_Psi2S_pThat-2_TuneCP5_HydjetDrumMB_5p02TeV_pythia8.root","read");
        if(state==1) rf = new TFile("../primary_input/OniaFlowSkim_Psi2S_JpsiTrig_Prompt_isMC1_HFNom_210603.root","read");
    ...
    for(int i=0; i<nEvt; i++){
        ...
        if (isMC && state==1) {TFile *wf = new TFile(Form("roots/OniaRooDataSet_isMC%d_PR_Psi_2S_20210727.root",isMC),"recreate");  wf->cd();}
        else if (isMC && state==2) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_BtoPsi_2S_20210727.root",isMC),"recreate");  wf->cd();}
        else if (!isMC) {TFile *wf = new TFile(Form("OniaRooDataSet_isMC%d_Psi_2S_%sw_Effw%d_Accw%d_PtW%d_TnP%d_20210727.root",isMC,outName.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");  wf->cd();}
        dataSet->Write();
    }
