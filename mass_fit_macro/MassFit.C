    ...
    )
{
  ...
  f1 = new TFile("/mnt/d/works/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/OniaRooDataSet_isMC0_JPsi_PRw_Effw1_Accw1_PtW1_TnP1_20210426.root");
  //f1 = new TFile("../skimmedFiles/OniaRooDataSet_isMC0_JPsi_w_Effw0_Accw0_PtW1_TnP1_202020210312.root");
  ...
  ws->var("mass")->Print();
  //cout << "ctau3D Range : " << ws->var("ctau3D")->getRange() << endl;
  //cout << "ctau3DErr Range : " << ws->var("ctau3DErr")->getRange() << endl;
  ...
  RooRealVar *sl2 = new RooRealVar("sl2","sl2", 0.5, -10., 10.);
  ...
  //Build the model
  RooRealVar *N_Jpsi= new RooRealVar("N_Jpsi","inclusive Jpsi signals",0,5000);
  RooRealVar *N_Bkg = new RooRealVar("N_Bkg","fraction of component 1 in bkg",0,50000);
  ...
  TFile* outFile;
  if(PR==2){
    outFile = new TFile(Form("../../roots/2DFit_%s/Mass/MassFitResult_%s_%s.root", DATE.Data(), bCont.Data(), kineLabel.Data()),"recreate");
    c_A->SaveAs(Form("../../figs/2DFit_%s/Mass/Mass_%s_%s.pdf", DATE.Data(), bCont.Data(), kineLabel.Data()));}
  pdfMASS_Tot->Write();
  datasetMass->Write();
  outFile->Close();
