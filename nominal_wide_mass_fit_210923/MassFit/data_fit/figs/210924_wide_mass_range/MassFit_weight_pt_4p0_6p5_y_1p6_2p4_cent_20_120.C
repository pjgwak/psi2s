...
{
    TString DATE = "210924_wide_mass_range";
	...
	TFile* f1 = new TFile("../../make_RooDataSet/roots/OniaRooDataSet_isMC0_pt_4.0_6.5_y_1.6_2.4_Cent_20_120_CtauCtu_0.0395_wPt1_wAccPt1_wTnPPR1_Psi_2S_210923.root","read");

    double massLow = 3.3;
    double massHigh = 4.3;
	TString kineCut;
	kineCut = Form("pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.3 && cBin>=%d && cBin<%d",ptLow, ptHigh, yLow, yHigh, cLow, cHigh);
    ...
    /*
     double sigma_1_init = 0.073;
     double x_init = 0.6341;
     double alpha_1_init = 2.0493;
     double n_1_init = 1.6168;
     double f_init = 0.6728;
     double m_lambda_init = 5.15;

     double sl1_mean = 0.559, sl2_mean = 0.147,  sl3_mean = 0.368;
     double N_Jpsi_high = 18000, N_Bkg_high = 2500000;
     */
	//         The order is {sigma_1,  x, alpha_1, n_1,   f, m_lambda}
	//pt3-4.5
    double paramsupper[8] = {0.2,  3.,   3, 3, 1,     25.0};
    double paramslower[8] = {0,   0,     0, 0, 0.0,     -5.0};
   
	//SIGNAL: initial params
    double sigma_1_init = 0.0705;
    double x_init = 0.6341;
    double alpha_1_init = 2.0493;
    double n_1_init = 1.6168;
    double f_init = 0.4042;
    double m_lambda_init = 5.15;

    double sl1_mean = 0.159, sl2_mean = 0.47,  sl3_mean = 0.368;
    double N_Jpsi_high = 20000, N_Bkg_high = 2500000;
	...
	// myPlot2_A->SetMinimum(3*10000);
	myPlot2_A->SetMaximum(1*100000);
    ...
    TFile* outFile;
    outFile = new TFile(Form("roots/%s/MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP),"recreate");
    c_A->SaveAs(Form("figs/%s/Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
